...
def extract_bboxes_keypoints(keypoints):
    """Compute bounding boxes from masks.
    mask: [height, width, num_instances]. Mask pixels are either 1 or 0.
    ...
    # Bounding box.
    vaild_indicies = np.where(keypoints[0, :, 2] > 0)[0]
    x1 = min(keypoints[0, :, 0][vaild_indicies]) - 30  # min width
    y1 = min(keypoints[0, :, 1][vaild_indicies]) - 30  # min height
    x2 = max(keypoints[0, :, 0][vaild_indicies]) + 30  # max width
    y2 = max(keypoints[0, :, 1][vaild_indicies]) + 30  # max height
...
class Dataset(object):
    ...
    def add_image(self, source, image_id, path, coord_list, **kwargs):
        image_info = {
            "id": image_id,
            "source": source,
            "path": path,
    ...
    def add_image_coco(self, source, image_id, path, **kwargs):
        image_info = {
            "id": image_id,
            "source": source,
            "path": path,
...
def resize_image(image, min_dim=None, max_dim=None, padding=False):
    ...
    h, w = image.shape[:2]
    window = (0, 0, max_dim-1, min_dim-1)
    scale_x = 1.0
    scale_y = 1.0
    # Scale?
    if min_dim:
        # Scale up but not down
        scale_x = max(1, min_dim / w)
    # Does it exceed max dim?
    if max_dim:
        scale_y = max(1, min_dim / h)
    # Resize image and mask
    if scale_x != 1 or scale_y != 1:
        image = scipy.misc.imresize(
            image, (round(h * scale_y), round(w * scale_x)))

    return image, window, [scale_x, scale_y]
...
def resize_keypoints(keypoint, new_size, scale_xy):
    ...
    for i in range(num_person):
        for j in range(num_keypoint):
            ...
            vis = keypoint[i, j, 2]
            #scale
            x = int(x*scale_xy[0] + 0.5)
            y = int(y*scale_xy[1] + 0.5)
            if(x >= new_size[1]):
                x = new_size[1] - 1
            if(y >= new_size[0]):
                y = new_size[0] - 1
...
def unmold_keypoint_mask(keypoints_prob, image_shape, keypoint_mask_shape = (56,56), keypoint_threshold= 0.08):
    """Converts a mask generated by the neural network into a format similar
    to it's original shape.
...
def unmold_keypoints_to_raw(keypoint,window, scale, shape):
    """Unmold a keypoint  using the given scale and padding.
       Typically, you get the scale and padding from resize_image() to
       ensure both, the image and the mask, are resized consistently.
       keypoint: [num_person, num_keypoint, 3]
