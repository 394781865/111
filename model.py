...
def build_fpn_keypoint_graph(rois, feature_maps,
    ...
    x = KL.TimeDistributed(
        KL.Lambda(lambda z: tf.image.resize_bilinear(z, [256, 256])), name="mrcnn_keypoint_mask_upsample_2")(x)
...
class MaskRCNN():
    ...
    def build(self, mode, config):
        ...
        mrcnn_feature_maps = [P2, P3, P4, P5, P6]
        ...
        if mode == "training":
            ...
            target_keypoint, target_keypoint_weight = DetectionKeypointTargetLayer\
                (config, name="proposal_targets")([target_rois, gt_keypoints])
    ...
    def unmold_keypoint_detections(self, detections, mrcnn_keypoints, image_shape, keypoint_threshold=0.05):
        """Reformats the detections of one image from the format of the neural
        network output to a format suitable for use in the rest of the
        application.
        ...
        for i in range(N):
            # Convert neural network mask to full size mask
            keypoint = utils.unmold_keypoint_mask(mrcnn_keypoints[i], image_shape, keypoint_mask_shape=[256, 256], keypoint_threshold=keypoint_threshold)
    ...
    def detect_keypoint(self, images):
        """Runs the detection pipeline.
        ...
        # Mold inputs to format expected by the neural network
        molded_images, windows, scales = self.mold_inputs(images)
        ...
        for i, image in enumerate(images):
            final_keypoints = \
                self.unmold_keypoint_detections(detections[i], mrcnn_keypoint_prob[i], image.shape, keypoint_threshold=self.config.KEYPOINT_THRESHOLD)
            results.append({
